...
class DES:
    ...
    class DESround:
        ...
        def expansion(halfBlock):
            ...
            return permute(halfBlock, expansionBox)
        ...
        def substitutionBox(keyMixedHalfBlock):
            ...
            for box in S_BOX:
                ...
                sboxRow = ((currentBits >> 5) << 1) + (currentBits & 0b100000)
                sboxCol = ((currentBits >> 4) << 3) + ((currentBits >> 3) << 2) + ((currentBits >> 2) << 1) + (currentBits & 0b10000)
                substitutionResults.append(S_BOX[sboxRow][sboxCol])
        ...
        def permutationBox(substitutionResults):
            ...
            for perm in P:
                ...
                if substitutionResults[quartet] >> bitOffset && 0b1:
                    permutationResult |= 1L << bitOffset
                else:
                    permutationResult &= ~(1L << bitOffset)
        ...
        def feistelFunction(rightHalf, roundKey):
            # 1 - expansion phase
            expandedRightHalf = expansion(rightHalf)

            # 2 - key mixing phase
            keyMixedExpandedHalfBlock = expandedRightHalf ^ roundKey

            # 3 - substitution phase
            substitutionResults = substitutionBox(keyMixedExpandedHalfBlock)

            # 4 - permutation phase
            permuted = permutationBox(substitutionResults)
        ...
        def applyRound(block, roundKey):
            leftHalf = block >> 32
            rightHalf = block & 2**32

            newRigtHalf = leftHalf ^ feistelFunction(rightHalf, roundKey)
    ...
    def generateRoundKeys(self):
        ...
        permutedKey = DESround.permute(self.key, initKeyPerm)
    ...
    def encrypt(self, messageString):
        ...
        for block in blocks:
            ...
            for _ in range(16):
                block = applyRound(block, currentRoundKey)
